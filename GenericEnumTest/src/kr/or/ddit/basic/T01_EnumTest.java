...
public class T01_EnumTest {
	 ...
	 * 
	 * 장점 : 기존 값만 비교가 아닌 타입까지 비교하여 안전한 코딩 가능하게 도움 줌
	 */
	 
	//City 열거형 객체 선언(기본값을 이용하는 열거형)
	public enum City{서울, 부산, 대구, 광주, 대전}
//	public enum Korea{서울, 부산, 대구, 광주, 대전}
	...
	public enum Season{
		...
		//시즌타입의 객체 봄,여름,가을,겨울 4개의 생성자를 str로 저장, enum의 생성자로 private만 허용, public 안됨 /  String, Int 등 교체가능
		Season(String data){
			this.str = data;
		}
...
//		  System.out.println(Korea.대전 == City.대전); 에러뜸
		  System.out.println(city == City.대전);
		  System.out.println(city == City.대구);

		  System.out.println("대구 => " + city.compareTo(City.대구)); //같다
		  System.out.println("대구 => " + city.compareTo(City.서울)); //서울이 2 먼저 선언됨
		  System.out.println("대구 => " + city.compareTo(City.대전)); //대전이 2 후에 선언됨
	}
}
